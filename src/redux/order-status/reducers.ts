...
import { FilterByStatusValuesAction } from './types';
...
const filterByStatusValuesReducer = (
  state: RequestState<FilterValues> = initState,
  action: FilterByStatusValuesAction = null,
): RequestState<FilterValues> => {
  switch (action.type) {
    case FILTER_BY_STATUS_VALUES_REQUESTED:
      return { ...state, pending: true };
    case FILTER_BY_STATUS_VALUES_RECIVED:
...
export { filterByStatusValuesReducer };
